...
import enum
from sqlalchemy import String, Float, Enum as SAEnum, ForeignKey, Integer, Text
...
class Aircraft(TimestampMixin, Base):
    ...
    status: Mapped[AircraftStatus] = mapped_column(
        SAEnum(AircraftStatus), nullable=False, default=AircraftStatus.available
    )
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)

    club: Mapped["Club"] = relationship(back_populates="aircraft")
    reservations: Mapped[list["Reservation"]
                         ] = relationship(back_populates="aircraft")
